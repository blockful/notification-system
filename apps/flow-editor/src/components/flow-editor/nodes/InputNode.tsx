...
function InputNodeComponent({ id, data, selected }: InputNodeProps) {
  ...
  return (
    <div className={`
      ...
      ${selected 
        ? 'border-cyan-500 shadow-lg shadow-cyan-500/20 ring-4 ring-cyan-500/10' 
        : 'border-cyan-200 shadow-md hover:shadow-lg hover:border-cyan-300'
      }
    `}>
      <BaseHandle 
        type="target" 
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-cyan-500 to-sky-500 px-4 py-2.5 flex items-center gap-2">
      ...
      {/* Source Key */}
      <div className="px-3 py-1.5 bg-cyan-50 border-b border-cyan-100">
        <code className="text-[11px] text-cyan-600 font-medium">
          {data.sourceKey}
        </code>
      </div>
      ...
      <div className="px-3 py-2 bg-slate-50 border-t border-slate-100">
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span className="italic">Waiting for user input...</span>
        </div>
      </div>
